...
const BEST_SELLERS: Product[] = Array.from({ length: 6 }).map((_, i) => ({
  id: `bs-${i}`,
  ...
  return (
    <>
      <Section id="products" className="py-24 bg-white border-y border-gray-100">
        ...
        <div className="flex gap-6 overflow-x-auto pb-12 snap-x snap-mandatory pt-4 px-4 -mx-4 md:px-0 md:mx-0 hide-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {BEST_SELLERS.map((product, i) => (
            <motion.div
              ...
              className="snap-start shrink-0 w-[280px] sm:w-[320px] group relative"
            >
              <div className="relative aspect-[4/5] bg-[#F5F5F7] rounded-3xl overflow-hidden mb-5 p-6 transition-all duration-300 group-hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)]">
                ...
                <button className="absolute top-4 right-4 z-10 text-gray-400 hover:text-red-500 transition-colors bg-white/80 p-2 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 duration-300 shadow-sm">
                  <Heart size={20} />
                </button>
                ...
                <div className="w-full h-full flex items-center justify-center pointer-events-none">
                  <img 
                    src={product.image}
                    alt={product.name}
                    className="w-4/5 h-4/5 object-contain mix-blend-darken transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-110 drop-shadow-xl"
                  />
                </div>

                {/* Animated "Add to Cart" button overlay */}
                <div className="absolute inset-x-4 bottom-4 translate-y-16 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 z-10">
                  <button 
